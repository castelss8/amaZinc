...
def SAT_MCP(n, m, sz, cpt, D):
    ...
    for j in range(n):
        solv.add(exactly_one([c[i][j] for i in range(m)]))
    ...
    for cour in range(m):
        ...
        solv.add(at_most_k(cour_weight, cpt[cour]))
    ...
    while time.time() < timeout:
        ...
        for courier in range(m):
            tmp_dist = [And(c[courier][item], pred[item][item_2]) for item in range(n) for item_2 in range(n) for _ in range(D[item_2][item])]
            tmp_dist += [And(c[courier][item], pred[item][n+courier]) for item in range(n) for _ in range(D[n][item])]
            tmp_dist += [And(c[courier][item], pred[n+courier][item]) for item in range(n) for _ in range(D[item][n])]
            solv.add(at_most_k(tmp_dist, best_obj-1))
        
        if solv.check()==sat:
            tmp_model = solv.model()
            item_pred, cour_item = [(i,j) for j in range(n+m) for i in range(n+m) if tmp_model.evaluate(pred[i][j])], [(i, j) for j in range(n) for i in range(m) if tmp_model.evaluate(c[i][j])] #tmp_model.evaluate(c[i][j][0])
